      ...
      <div className="container 2xl:max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-20">
          {stats.map((stat, index) => (
            <div
              key={stat.label}
              className={`glass-card p-6 text-center transition-all duration-500 hover:border-codecraft-accent-indigo/30 ${
                isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
              }`}
              style={{ transitionDelay: `${index * 100}ms` }}
            >
              <div className="w-12 h-12 rounded-xl bg-gradient-linear flex items-center justify-center mx-auto mb-4">
